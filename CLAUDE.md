...
1. **Networking (`src/networking/`)**:
   ...
   - Pipeline batching (`src/storage/batch.rs`): consecutive-group strategy annotates each command with `(original_index, shard_index, is_read)`, groups consecutive same-shard commands into batches under a single lock acquisition per shard, then reorders responses back to original command order. Commands are NOT sorted across shards to preserve client send order (required when maxmemory is configured). Works with RESP2/RESP3 and security-enabled connections (ACL checks per-command within batch loop)
