...
use crate::replication::{ReplicationState, ReplicaInfo};
...
impl ReplicationServer {
            ...
            match listener.accept().await {
                Ok((socket, addr)) => {
                    ...
                    tokio::spawn(async move {
                        if let Err(e) = Self::handle_replica(socket, addr, engine, state, replica_channels, buffer_size).await {
    ...
    async fn handle_replica(
        ...
        info!("New replica connection from {}, assigned ID: {}", addr, replica_id);
        
        // Process the initial handshake
        let mut replica_info = Self::process_handshake(&mut socket, &mut buffer, &replica_id, addr).await?;
        ...
        let writer_task = tokio::spawn(async move {
            while let Some(cmd) = rx.recv().await {
                if let Err(e) = Self::send_command(&mut writer_socket, cmd).await {
                    error!("Error sending command to replica {}: {}", replica_id_clone, e);
                    break;
                }
            ...
            while !buffer.is_empty() {
                match RespValue::parse(&mut buffer) {
                            ...
                            Ok(cmd) => {
                                ...
                                match cmd.name.to_uppercase().as_str() {
                                    "REPLCONF" => {
                                        ...
                                        Self::handle_replconf(&mut socket, &cmd.args, &mut replica_info).await?;
                                    }
                                    "PSYNC" => {
                                        // Handle PSYNC command
                                        Self::handle_psync(&mut socket, &cmd.args, &replica_info).await?;
                                    }
                                    _ => {
                                        ...
                                        let bytes = ok_response.serialize().map_err(|e| {
                                            ReplicationError::Protocol(format!("Failed to serialize response: {}", e))
                                        })?;
                                        socket.write_all(&bytes).await.map_err(ReplicationError::Io)?;
                                    }
                                }
                    ...
                    Ok(None) => {
                        ...
                        debug!("Incomplete message from replica {}, waiting for more data", addr);
                        break;
                    }
                    Err(e) => {
    ...
    async fn process_handshake(
        ...
        loop {
            ...
            match read_result {
                Ok(0) => {
                    // Connection closed by replica
                    return Err(ReplicationError::Connection("Connection closed by replica during handshake".to_string()));
                }
                Ok(_) => {
                            ...
                            match crate::command::parser::parse_command(value) {
                                Ok(cmd) => {
                                    if cmd.name.to_uppercase() == "PING" {
                                        // Respond with PONG
                                        let pong_response = RespValue::SimpleString("PONG".to_string());
                                        let bytes = pong_response.serialize().map_err(|e| {
                                            ReplicationError::Protocol(format!("Failed to serialize PONG: {}", e))
                                        })?;
                                        ...
                                        // Unexpected command
                                        return Err(ReplicationError::Protocol(format!("Expected PING, got: {}", cmd.name)));
                                    }
                                }
                                Err(e) => {
                                    return Err(ReplicationError::Protocol(format!("Command parsing error: {}", e)));
                                }
                            }
                        }
                        Ok(None) => {
                            ...
                            match crate::command::parser::parse_command(value) {
                                Ok(cmd) => {
                                    if cmd.name.to_uppercase() == "REPLCONF" {
                                        ...
                                        let ok_response = RespValue::SimpleString("OK".to_string());
                                        let bytes = ok_response.serialize().map_err(|e| {
                                            ReplicationError::Protocol(format!("Failed to serialize OK: {}", e))
                                        })?;
                                        socket.write_all(&bytes).await?;
                                        ...
                                        // Unexpected command
                                        return Err(ReplicationError::Protocol(format!("Expected REPLCONF, got: {}", cmd.name)));
                                    }
                                }
                                Err(e) => {
                                    return Err(ReplicationError::Protocol(format!("Command parsing error: {}", e)));
    ...
    async fn send_initial_sync(socket: &mut (impl AsyncWrite + Unpin), engine: &Arc<StorageEngine>) -> Result<(), ReplicationError> {
    ...
    async fn handle_psync(
        ...
        // Parse the arguments
        if args.len() < 2 {
            return Err(ReplicationError::Protocol("PSYNC requires at least 2 arguments".to_string()));
        }
        
        let replica_id = String::from_utf8_lossy(&args[0]);
        let offset = String::from_utf8_lossy(&args[1]);
        
        debug!("Received PSYNC from replica {}, offset: {}", replica_id, offset);
    ...
    async fn send_command(socket: &mut (impl AsyncWrite + Unpin), command: RespCommand) -> Result<(), ReplicationError> {
        ...
        cmd_parts.extend(command.args.into_iter().map(|arg| RespValue::BulkString(Some(arg))));
