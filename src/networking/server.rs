...
impl Server {
    ...
    pub fn with_persistence(mut self, persistence: Arc<PersistenceManager>, sync_policy: AofSyncPolicy) -> Self {
                    ...
                    match self.connection_limit.clone().try_acquire_owned() {
                        Ok(permit) => {
                            ...
                            tokio::spawn(async move {
                                ...
                                if let Err(e) = Self::handle_client(socket, cmd_handler, persistence, security, aof_sync_policy, buffer_size).await {
        ...
        loop {
            ...
            match read_result {
                ...
                Ok(n) => {
                    ...
                    logging::log_protocol_data(&client_addr_str, "request", &buffer[buffer.len()-n..], true);
                },
                Err(e) => {
            ...
            // CRITICAL FIX: Create a separate buffer for command parsing
            // This prevents response data from contaminating command parsing
            let mut command_buffer = buffer.clone();
            
            // Process complete RESP messages from the command buffer only
            while !command_buffer.is_empty() {
                // If we have leftover data from previous read, prepend it to the command buffer
                if !partial_command_buffer.is_empty() {
                    let mut combined = BytesMut::with_capacity(partial_command_buffer.len() + command_buffer.len());
                    combined.extend_from_slice(&partial_command_buffer);
                    combined.extend_from_slice(&command_buffer);
                    command_buffer = combined;
                    partial_command_buffer.clear();
                    debug!(client_addr = %client_addr_str, combined_buffer = command_buffer.len(), "Combined partial buffer with new data");
                }
                
                // Try the fast path for common commands first
                match RespValue::try_parse_common_command(&mut command_buffer) {
                    Ok(Some(resp_cmd)) => {
                        ...
                        if let Some(ref security_mgr) = security {
                            if !is_auth_command && security_mgr.require_auth() && !security_mgr.is_authenticated(&client_addr_str) {
                                let error_msg = "NOAUTH Authentication required.";
                                Self::send_error(&mut socket, error_msg.to_string(), &client_addr_str).await?;
                                continue;
                            }
                        }
                        ...
                        if let Some(ref persistence_mgr) = persistence {
                            if let Err(e) = persistence_mgr.log_command(resp_cmd.clone(), aof_sync_policy).await {
                        ...
                        match command_handler.process(cmd.clone()).await {
                            Ok(response) => {
                                ...
                                if let Ok(bytes) = response.serialize() {
                                    ...
                                    Self::send_response(&mut socket, response, &client_addr_str).await?;
                                }
                            },
                            Err(err) => {
                                // Send error response instead of dropping the connection
                                Self::send_error(&mut socket, err.to_string(), &client_addr_str).await?;
                            }
                        }
                    ...
                    Ok(None) => {
                        ...
                        match RespValue::parse(&mut command_buffer) {
                            Ok(Some(value)) => {
                                ...
                                match parser::parse_command(value) {
                                    Ok(cmd) => {
                                        let cmd_name_str = String::from_utf8_lossy(cmd.name.as_bytes()).to_string();
                                    ...
                                    Err(e) => {
                                        ...
                                        Self::send_error(&mut socket, e.to_string(), &client_addr_str).await?;
                                    }
                                }
                            },
                            Ok(None) => {
                                debug!(client_addr = %client_addr_str, buffer_len = command_buffer.len(), "Incomplete RESP message, waiting for more data");
                                // Save the partial data for the next iteration
                                partial_command_buffer.extend_from_slice(&command_buffer);
                            ...
                            Err(e) => {
                                ...
                                if command_buffer.len() > 1024 * 50 {
                                    debug!(client_addr = %client_addr_str, buffer_len = command_buffer.len(), 
                                           "Large buffer in error state, checking for JSON");
                                    
                                    // Check if this seems to be JSON data
                                    if command_buffer.len() > 5 && (command_buffer[0] == b'{' || command_buffer[0] == b'[') {
                    ...
                    Err(e) => {
                        ...
                        if let RespError::Incomplete = e {
                            debug!(client_addr = %client_addr_str, buffer_len = command_buffer.len(), "Incomplete RESP message in fast path, waiting for more data");
                            partial_command_buffer.extend_from_slice(&command_buffer);
                            command_buffer.clear();
                            break;
                        } else {
                            ...
                            Self::send_error(&mut socket, error_msg, &client_addr_str).await?;
                            // Clear command buffer to start fresh after error
                            command_buffer.clear();
                            continue;
                        }
                    }
                ...
                if commands_processed >= MAX_BATCH_COMMANDS || response_buffer.len() >= MAX_BATCH_SIZE {
                    break;
                }
            }
            
            // CRITICAL FIX: Update the main buffer to reflect consumed commands
            // Calculate how much data was consumed from the command buffer
            let original_len = buffer.len();
            let consumed = original_len - command_buffer.len();
            if consumed > 0 {
                buffer.advance(consumed);
                debug!(client_addr = %client_addr_str, consumed_bytes = consumed, 
                       remaining_bytes = buffer.len(), "Advanced main buffer after command processing");
            }
            ...
            if !response_buffer.is_empty() {
                ...
                logging::log_protocol_data(&client_addr_str, "batched_response", &response_buffer, false);
    ...
    async fn send_response(socket: &mut TcpStream, response: RespValue, client_addr: &str) -> Result<(), NetworkError> {
        match response.serialize() {
            ...
            Err(e) => {
                ...
                Self::send_error(socket, error_msg, client_addr).await
    ...
    async fn send_error(socket: &mut TcpStream, error_message: String, client_addr: &str) -> Result<(), NetworkError> {
