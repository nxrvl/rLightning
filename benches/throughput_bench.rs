...
fn bench_string_operations_throughput(c: &mut Criterion) {
    ...
    for concurrency in [1, 10, 50, 100].iter() {
        group.bench_with_input(BenchmarkId::new("SET Operations", concurrency), concurrency, |b, &concurrency| {
...
fn bench_hash_operations_throughput(c: &mut Criterion) {
    ...
    for concurrency in [1, 10, 50].iter() {
        group.bench_with_input(BenchmarkId::new("HSET Operations", concurrency), concurrency, |b, &concurrency| {
            let runtime = tokio::runtime::Runtime::new().unwrap();
...
fn bench_list_operations_throughput(c: &mut Criterion) {
    ...
    for concurrency in [1, 10, 50].iter() {
        group.bench_with_input(BenchmarkId::new("LPUSH Operations", concurrency), concurrency, |b, &concurrency| {
            let runtime = tokio::runtime::Runtime::new().unwrap();
...
fn bench_set_operations_throughput(c: &mut Criterion) {
    ...
    for concurrency in [1, 10, 50].iter() {
        group.bench_with_input(BenchmarkId::new("SADD Operations", concurrency), concurrency, |b, &concurrency| {
            let runtime = tokio::runtime::Runtime::new().unwrap();
...
fn bench_sorted_set_operations_throughput(c: &mut Criterion) {
    ...
    for concurrency in [1, 10, 50].iter() {
        group.bench_with_input(BenchmarkId::new("ZADD Operations", concurrency), concurrency, |b, &concurrency| {
            let runtime = tokio::runtime::Runtime::new().unwrap();
...
fn bench_mixed_workload_throughput(c: &mut Criterion) {
    ...
    for concurrency in [10, 50, 100].iter() {
        group.bench_with_input(BenchmarkId::new("Mixed Operations", concurrency), concurrency, |b, &concurrency| {
            let runtime = tokio::runtime::Runtime::new().unwrap();
                        ...
                        for i in 0..500 {
                            ...
                            let handle = tokio::spawn(async move {
                                ...
                                let cmd = match operation_type {
                                    ...
                                    4..=8 => { // 50% GET operations
                                        let key = format!("initial_key{}", i % 100).into_bytes();
                                        Command {
                                            name: "get".to_string(),
                                            args: vec![key],
                                        }
                                    },
                                    9 => { // 10% other operations (INCR, HSET, LPUSH, etc.)
                                        match i % 5 {
                                            0 => {
                                                let key = format!("counter{}", i % 10).into_bytes();
                                                Command {
                                                    name: "incr".to_string(),
                                                    args: vec![key],
                                                }
                                            ...
                                            1 => {
                                                let key = format!("hash{}", i % 10).into_bytes();
                                                let field = format!("field{}", i).into_bytes();
                                            ...
                                            2 => {
                                                let key = format!("list{}", i % 10).into_bytes();
                                                let value = format!("item{}", i).into_bytes();
                                                Command {
                                                    name: "lpush".to_string(),
                                                    args: vec![key, value],
                                            ...
                                            _ => {
                                                let key = format!("zset{}", i % 10).into_bytes();
                                                let score = format!("{}.0", i).into_bytes();
                                                let member = format!("zmember{}", i).into_bytes();
                                                Command {
                                                    name: "zadd".to_string(),
                                                    args: vec![key, score, member],
                                                }
...
fn bench_expiry_performance(c: &mut Criterion) {
    ...
    for concurrency in [1, 10, 50].iter() {
        group.bench_with_input(BenchmarkId::new("EXPIRE Operations", concurrency), concurrency, |b, &concurrency| {
            b.iter_with_setup(
                || (),
                |_| {
                    runtime.block_on(async {
...
fn bench_large_values_throughput(c: &mut Criterion) {
    ...
    for &value_size in &[1_000, 10_000, 50_000] {
        group.bench_with_input(BenchmarkId::new("Large SET Operations", value_size), &value_size, |b, &value_size| {
            b.iter_with_setup(
                || (),
                |_| {
                    runtime.block_on(async {
                        ...
                        for i in 0..50 { // Fewer operations for large values
                            let handler_ref = Arc::clone(&handler);
                            let semaphore_ref = Arc::clone(&semaphore);
...
fn bench_memory_usage_patterns(c: &mut Criterion) {
    ...
    for &memory_limit_mb in &[32, 64, 128] {
        group.bench_with_input(BenchmarkId::new("Memory Pressure", memory_limit_mb), &memory_limit_mb, |b, &memory_limit_mb| {
            b.iter_with_setup(
                || (),
                |_| {
                    runtime.block_on(async {
...
criterion_group!(
    string_throughput,
    bench_string_operations_throughput
);
