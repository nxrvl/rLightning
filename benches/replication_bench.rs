...
fn bench_replication_command_propagation(c: &mut Criterion) {
    ...
    for concurrency in [1, 5, 10].iter() {
        group.bench_with_input(BenchmarkId::new("Master command processing", concurrency), concurrency, |b, &concurrency| {
            let runtime = tokio::runtime::Runtime::new().unwrap();
...
fn bench_replication_memory_usage(c: &mut Criterion) {
    ...
    for &data_size in &[100, 500, 1000] {
        group.bench_with_input(BenchmarkId::new("Memory usage with replicas", data_size), &data_size, |b, &data_size| {
            let runtime = tokio::runtime::Runtime::new().unwrap();
...
fn bench_replication_lag_simulation(c: &mut Criterion) {
    ...
    for &command_rate in &[10, 50, 100] {
        group.bench_with_input(BenchmarkId::new("Command processing under load", command_rate), &command_rate, |b, &command_rate| {
            let runtime = tokio::runtime::Runtime::new().unwrap();
