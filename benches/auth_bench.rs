...
fn bench_auth_command_overhead(c: &mut Criterion) {
    ...
    for &auth_enabled in &[false, true] {
        let auth_label = if auth_enabled {
            "with_auth"
        } else {
            "without_auth"
        };
        ...
        group.bench_with_input(
            BenchmarkId::new("SET command", auth_label),
            &auth_enabled,
            |b, &auth_enabled| {
                async_bench_with_setup!(
                    ...
                    async {
                        ...
                        if auth_enabled {
                            ...
                            storage
                                .set(b"auth:enabled".to_vec(), b"true".to_vec(), None)
                                .await
                                .unwrap();
                        }
...
fn bench_auth_session_management(c: &mut Criterion) {
    ...
    group.bench_function("Session validation", |b| {
        ...
        rt.block_on(async {
            for i in 0..500 {
                ...
                let session_data =
                    format!("{{\"user\":\"user_{}\",\"created\":{}}}", i, 1600000000);
                storage
                    .set(
        ...
        async_bench!(b, async {
            ...
            let _session_data = storage
                .get(black_box(session_key.as_bytes()))
                .await
                .unwrap();
        });
    });
    ...
    group.bench_function("Session cleanup", |b| {
        async_bench!(b, async {
            ...
            let session_keys: Vec<_> = all_keys
                .iter()
...
fn bench_auth_concurrent_operations(c: &mut Criterion) {
    ...
    for concurrency in [10, 25].iter() {
        // Reduced concurrency levels
        group.bench_with_input(
            BenchmarkId::new("Concurrent auth checks", concurrency),
            concurrency,
            |b, &concurrency| {
                async_bench_with_setup!(
                    b,
...
fn bench_auth_memory_usage(c: &mut Criterion) {
    ...
    for &user_count in &[50, 100, 200] {
        // Reduced user counts
        group.bench_with_input(
            BenchmarkId::new("Memory usage with users", user_count),
            &user_count,
            |b, &user_count| {
                async_bench!(b, async {
                    let storage_config = StorageConfig::default();
                    ...
                    for i in 0..user_count {
                        ...
                        // User session
                        let session_data =
                            format!("{{\"user\":\"{}\",\"created\":{}}}", user_id, 1600000000);
                        storage
                            .set(
...
criterion_group!(auth_overhead, bench_auth_command_overhead);
...
criterion_group!(auth_scalability, bench_auth_memory_usage);
