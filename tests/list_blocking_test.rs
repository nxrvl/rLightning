...
async fn test_lpushx_rpushx_integration() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Verify order: [a, x, z]
    let resp = client.send_command_str("LRANGE", &["mylist", "0", "-1"]).await?;
...
async fn test_linsert_integration() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Create list [a, b, d]
    client.send_command_str("RPUSH", &["ilist", "a", "b", "d"]).await?;

    // Insert "c" before "d"
    let resp = client.send_command_str("LINSERT", &["ilist", "BEFORE", "d", "c"]).await?;
    assert_eq!(resp, RespValue::Integer(4));

    // Insert "e" after "d"
    let resp = client.send_command_str("LINSERT", &["ilist", "AFTER", "d", "e"]).await?;
    assert_eq!(resp, RespValue::Integer(5));

    // Verify: [a, b, c, d, e]
    let resp = client.send_command_str("LRANGE", &["ilist", "0", "-1"]).await?;
    if let RespValue::Array(Some(values)) = resp {
    ...
    // Pivot not found
    let resp = client.send_command_str("LINSERT", &["ilist", "BEFORE", "z", "x"]).await?;
...
async fn test_lset_integration() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Create list [a, b, c]
    client.send_command_str("RPUSH", &["slist", "a", "b", "c"]).await?;

    // Set index 1
    let resp = client.send_command_str("LSET", &["slist", "1", "B"]).await?;
    ...
    // Negative index
    let resp = client.send_command_str("LSET", &["slist", "-1", "C"]).await?;
    assert_eq!(resp, RespValue::SimpleString("OK".to_string()));
...
async fn test_lpos_integration() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    client.send_command_str("RPUSH", &["plist", "a", "b", "c", "b", "a"]).await?;
    ...
    let resp = client.send_command_str("LPOS", &["plist", "b", "COUNT", "0"]).await?;
    if let RespValue::Array(Some(values)) = resp {
...
async fn test_lmove_integration() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Create source [a, b, c]
    client.send_command_str("RPUSH", &["msrc", "a", "b", "c"]).await?;

    // Move left of src to right of dst
    let resp = client.send_command_str("LMOVE", &["msrc", "mdst", "LEFT", "RIGHT"]).await?;
    assert_eq!(resp, RespValue::BulkString(Some(b"a".to_vec())));

    // src should be [b, c], dst should be [a]
    let resp = client.send_command_str("LRANGE", &["msrc", "0", "-1"]).await?;
    if let RespValue::Array(Some(values)) = resp {
    ...
    let resp = client.send_command_str("LRANGE", &["mdst", "0", "-1"]).await?;
    if let RespValue::Array(Some(values)) = resp {
...
async fn test_lmpop_integration() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Create lists
    client.send_command_str("RPUSH", &["mp1", "a", "b", "c"]).await?;
    client.send_command_str("RPUSH", &["mp2", "x", "y"]).await?;

    // Pop 1 from left
    let resp = client.send_command_str("LMPOP", &["2", "mp1", "mp2", "LEFT"]).await?;
    if let RespValue::Array(Some(values)) = resp {
    ...
    let resp = client.send_command_str("LMPOP", &["2", "mp1", "mp2", "RIGHT", "COUNT", "2"]).await?;
    if let RespValue::Array(Some(values)) = resp {
        assert_eq!(values[0], RespValue::BulkString(Some(b"mp1".to_vec())));
        if let RespValue::Array(Some(elements)) = &values[1] {
...
async fn test_blpop_immediate_integration() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    client.send_command_str("RPUSH", &["blist", "hello", "world"]).await?;
...
async fn test_blpop_timeout_integration() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let resp = client.send_command_str("BLPOP", &["emptykey", "0.2"]).await?;
    let elapsed = start.elapsed();

    assert_eq!(resp, RespValue::Array(None));
    assert!(elapsed >= Duration::from_millis(150), "Expected timeout, elapsed: {:?}", elapsed);
...
async fn test_blpop_wakeup_integration() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let handle = tokio::spawn(async move {
        let resp = client1.send_command_str("BLPOP", &["wakekey", "5"]).await;
        resp
    });
    ...
    client2.send_command_str("RPUSH", &["wakekey", "wakedata"]).await?;
...
async fn test_brpop_immediate_integration() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    client.send_command_str("RPUSH", &["brlist", "a", "b"]).await?;
...
async fn test_blmove_immediate_integration() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    client.send_command_str("RPUSH", &["bmsrc", "a", "b"]).await?;

    let resp = client.send_command_str("BLMOVE", &["bmsrc", "bmdst", "LEFT", "RIGHT", "1"]).await?;
    assert_eq!(resp, RespValue::BulkString(Some(b"a".to_vec())));

    // Verify dst
    let resp = client.send_command_str("LRANGE", &["bmdst", "0", "-1"]).await?;
    if let RespValue::Array(Some(values)) = resp {
        assert_eq!(values.len(), 1);
        assert_eq!(values[0], RespValue::BulkString(Some(b"a".to_vec())));
    } else {
...
async fn test_blmpop_immediate_integration() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    client.send_command_str("RPUSH", &["bmplist", "a", "b"]).await?;

    let resp = client.send_command_str("BLMPOP", &["1", "1", "bmplist", "LEFT"]).await?;
    if let RespValue::Array(Some(values)) = resp {
...
async fn test_blpop_multi_key_integration() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let resp = client.send_command_str("BLPOP", &["mk1", "mk2", "1"]).await?;
    if let RespValue::Array(Some(values)) = resp {
