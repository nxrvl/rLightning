...
async fn test_large_json_set_get() {
    ...
    let server = Server::new_with_storage(server_addr, store);
    ...
    let array_elements: Vec<String> = (0..20).map(|i| i.to_string()).collect();
    json_data.push_str(&array_elements.join(", "));
    json_data.push_str("]\n  },\n");
