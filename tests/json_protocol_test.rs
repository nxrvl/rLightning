...
async fn test_large_json_set_get() {
    ...
    json_data.push_str(&format!("  \"control_chars\": \"Text with control chars: \\u001F separator \\u001E record\",\n"));
    ...
    match set_response {
        ...
        _ => {
            panic!("JSON.SET failed with unexpected response: {:?}", set_response);
        }
    }
    ...
    match get_response {
        RespValue::BulkString(Some(data)) => {
            ...
            assert!(retrieved_json.len() > 5000, "Retrieved JSON should be at least 5KB");
            assert!(retrieved_json.starts_with("{"), "Retrieved JSON should start with {{");
            assert!(retrieved_json.ends_with("}"), "Retrieved JSON should end with }}");
            
            // Parse the JSON to ensure it's valid
            let parsed: serde_json::Value = serde_json::from_str(&retrieved_json).expect("Retrieved JSON should be valid");
            
            // Verify some of the content
            assert!(parsed["field_1"].is_string(), "Field_1 should be a string");
            assert!(parsed["nested"]["array"].is_array(), "Nested array should be an array");
            
            // Check control characters were handled
            let control_chars_text = parsed["control_chars"].as_str().unwrap();
            assert!(control_chars_text.contains("control chars"), "Control chars text should be present");
        },
        _ => {
            panic!("JSON.GET failed with unexpected response: {:?}", get_response);
        }
    }
}
...
async fn test_python_redis_client_format() {
    ...
    // Add fields to make it approximately 5.4KB
    for i in 0..100 {
        json_data.push_str(&format!("  \"field_{}\": \"This is test data for field {}. Adding text to make it larger.\",\n", i, i));
    }
    ...
    json_data.push_str("    \"description\": \"This is a test story for Redis protocol testing.\",\n");
    ...
    match get_response {
        RespValue::BulkString(Some(data)) => {
            let retrieved_json = String::from_utf8_lossy(&data).to_string();
            println!("Retrieved JSON for Python-style key: {} bytes", retrieved_json.len());
...
async fn test_json_special_characters() {
    ...
    for (test_name, json_data) in test_cases {
        ...
        assert!(matches!(set_response, RespValue::SimpleString(ref s) if s == "OK"),
                "JSON.SET should succeed for {}", test_name);
        ...
        match get_response {
            RespValue::BulkString(Some(data)) => {
                ...
                assert_eq!(original, retrieved_parsed, "JSON should match for {}", test_name);
            },
