...
async fn test_redis_transactions() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    match multi_result {
        RespValue::SimpleString(response) => {
            ...
            let set1_result = client
                .send_command_str("SET", &["tx_key1", "value1"])
                .await?;
            let set2_result = client
                .send_command_str("SET", &["tx_key2", "value2"])
                .await?;
            ...
            if let RespValue::Array(Some(results)) = exec_result {
                assert_eq!(
                    results.len(),
                    3,
                    "EXEC should return results for all 3 commands"
                );
                ...
                assert_eq!(
                    ...
                    "First command (SET) should return OK"
                );
                assert_eq!(
                    results[1],
                    RespValue::SimpleString("OK".to_string()),
                    "Second command (SET) should return OK"
                );
                assert_eq!(
                    results[2],
                    RespValue::Integer(1),
                    "Third command (INCR) should return 1"
                );
    ...
    // Queue commands in the transaction
    client
        .send_command_str("SET", &["should_not_be_set", "value"])
        .await?;
    client
        .send_command_str("INCR", &["should_not_be_incremented"])
        .await?;
    ...
    match discard_result {
        RespValue::SimpleString(response) => {
            ...
            let get_result = client
                .send_command_str("GET", &["should_not_be_set"])
                .await?;
            assert_eq!(
                get_result,
                RespValue::BulkString(None),
                "Key should not exist after DISCARD"
            );

            let get_counter_result = client
                .send_command_str("GET", &["should_not_be_incremented"])
                .await?;
            assert_eq!(
                get_counter_result,
                RespValue::BulkString(None),
                "Counter should not exist after DISCARD"
            );
        }
        RespValue::Error(ref err) => {
            if err.contains("unknown command") || err.contains("not implemented") {
                ...
                panic!(
                    "Unexpected error response from DISCARD: {:?}",
                    discard_result
                );
            }
        }
        _ => {
            panic!(
                "Unexpected response type from DISCARD: {:?}",
                discard_result
            );
        }
    }
    ...
    client
        .send_command_str("SET", &["error_test_key", "value"])
        .await?;
    ...
    client
        .send_command_str("SET", &["another_key", "value"])
        .await?;
    ...
    match exec_result {
        RespValue::Array(Some(results)) => {
            ...
            assert!(
                matches!(results[1], RespValue::Error(ref e) if e.contains("not an integer") || e.contains("WRONGTYPE")),
                "INCR on non-numeric string should fail: got {:?}",
                results[1]
            );
            ...
            assert_eq!(
                results[2],
                RespValue::SimpleString("OK".to_string()),
                "Third SET should still succeed"
            );
        ...
        RespValue::Error(err) => {
            ...
            println!(
                "EXEC returned error: {}. Some implementations abort the transaction on errors.",
                err
            );
        }
        _ => {
    ...
    // Set initial value
    client
        .send_command_str("SET", &["isolation_test", "initial"])
        .await?;
    ...
    client
        .send_command_str("SET", &["isolation_test", "updated_in_tx"])
        .await?;
    ...
    client2
        .send_command_str("SET", &["isolation_test", "updated_by_client2"])
        .await?;
    ...
    if let RespValue::Array(Some(results)) = exec_result {
        assert_eq!(
            results[0],
            RespValue::SimpleString("OK".to_string()),
            "SET in transaction should succeed"
        );
        ...
        assert_eq!(
            get_result,
            RespValue::BulkString(Some(b"updated_in_tx".to_vec())),
    ...
    client
        .send_command_str("SET", &["user:1:name", "Alice"])
        .await?;
    client
        .send_command_str("SET", &["user:1:balance", "100"])
        .await?;
    client
        .send_command_str("SET", &["user:2:name", "Bob"])
        .await?;
    client
        .send_command_str("SET", &["user:2:balance", "50"])
        .await?;
    ...
    client
        .send_command_str("DECRBY", &["user:1:balance", "30"])
        .await?;
    client
        .send_command_str("INCRBY", &["user:2:balance", "30"])
        .await?;
    client
        .send_command_str("SADD", &["transactions", "transfer:1:2:30"])
        .await?;
    ...
    if let RespValue::Array(Some(results)) = exec_result {
        assert_eq!(
            results.len(),
            3,
            "Complex transaction should execute all 3 commands"
        );
        ...
        assert_eq!(
            balance1_result,
            RespValue::BulkString(Some(b"70".to_vec())),
            "User 1 balance should be 70"
        );

        let balance2_result = client.send_command_str("GET", &["user:2:balance"]).await?;
        assert_eq!(
            balance2_result,
            RespValue::BulkString(Some(b"80".to_vec())),
            "User 2 balance should be 80"
        );

        let transaction_log = client
            .send_command_str("SMEMBERS", &["transactions"])
            .await?;
        if let RespValue::Array(Some(members)) = transaction_log {
            ...
            if let RespValue::BulkString(Some(entry)) = &members[0] {
                assert_eq!(
                    ...
                    "transfer:1:2:30",
                    "Transaction log entry should be correct"
                );
            }
        }
    }
    ...
    client
        .send_command_str("SET", &["watched_key", "original"])
        .await?;
    ...
    match watch_result {
        RespValue::SimpleString(response) => {
            ...
            client2
                .send_command_str("SET", &["watched_key", "modified"])
                .await?;
            ...
            client
                .send_command_str("SET", &["watched_key", "from_tx"])
                .await?;
            ...
            assert_eq!(
                get_result,
                RespValue::BulkString(Some(b"modified".to_vec())),
    ...
    client
        .send_command_str("SET", &["watched_key2", "original"])
        .await?;
    ...
    // Queue a command in the transaction
    client
        .send_command_str("SET", &["watched_key2", "from_tx"])
        .await?;
    ...
    if let RespValue::Array(Some(results)) = exec_result {
        ...
        assert_eq!(
            get_result,
            RespValue::BulkString(Some(b"from_tx".to_vec())),
    ...
    // Create a key for WATCH
    client
        .send_command_str("SET", &["unwatched_key", "original"])
        .await?;
    ...
    // Modify the key from another client
    client2
        .send_command_str("SET", &["unwatched_key", "modified"])
        .await?;
    ...
    match unwatch_result {
        RespValue::SimpleString(response) => {
            ...
            // Queue a command in the transaction
            client
                .send_command_str("SET", &["unwatched_key", "from_tx"])
                .await?;
        ...
        RespValue::Error(ref err) => {
            if err.contains("unknown command") || err.contains("not implemented") {
                ...
                panic!(
                    "Unexpected error response from UNWATCH: {:?}",
                    unwatch_result
                );
            }
        }
        _ => {
            panic!(
                "Unexpected response type from UNWATCH: {:?}",
                unwatch_result
            );
        }
    }
