...
async fn test_redis_nested_data_structures() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    client.send_command_str("SET", &["user:1:id", "u12345"]).await?;
    client.send_command_str("SET", &["user:1:username", "johndoe"]).await?;
    client.send_command_str("HSET", &["user:1:profile", "first_name", "John", "last_name", "Doe", "age", "30"]).await?;
    client.send_command_str("HSET", &["user:1:settings", "theme", "dark", "notifications", "on"]).await?;
    client.send_command_str("SADD", &["user:1:roles", "admin", "editor", "viewer"]).await?;
    client.send_command_str("ZADD", &["user:1:scores", "100", "game1", "200", "game2", "150", "game3"]).await?;
    client.send_command_str("LPUSH", &["user:1:activity", "login:2023-01-01", "purchase:2023-01-15", "login:2023-02-01"]).await?;
    ...
    let response = client.send_command_str("HGET", &["user:1:profile", "first_name"]).await?;
    ...
    client.send_command_str("SET", &["user:1:email_domain_com", "john@example.com"]).await?;
    client.send_command_str("SET", &["user:1:address_home", "123 Main St"]).await?;
    client.send_command_str("SET", &["user:1:data_json", "{\"key\":\"value\"}"]).await?;
    
    // Verify the keys we set (using the underscore version)
    let response = client.send_command_str("GET", &["user:1:email_domain_com"]).await?;
    assert_eq!(response, RespValue::BulkString(Some(b"john@example.com".to_vec())));
    
    let response = client.send_command_str("GET", &["user:1:address_home"]).await?;
    assert_eq!(response, RespValue::BulkString(Some(b"123 Main St".to_vec())));
    ...
    for key in keys_to_fetch {
        ...
        assert!(matches!(response, RespValue::BulkString(Some(_))), "Expected value for {}", key);
    }
    ...
    // Add root nodes
    client.send_command_str("SADD", &["tree:roots", "root1", "root2"]).await?;
    
    // Add children for root1
    client.send_command_str("SADD", &["tree:children:root1", "node1", "node2", "node3"]).await?;
    
    // Add children for root2
    client.send_command_str("SADD", &["tree:children:root2", "node4", "node5"]).await?;
    
    // Add children for node1
    client.send_command_str("SADD", &["tree:children:node1", "leaf1", "leaf2"]).await?;
    
    // Store node data
    client.send_command_str("HSET", &["tree:data:root1", "type", "folder", "name", "Root 1", "created", "2023-01-01"]).await?;
    client.send_command_str("HSET", &["tree:data:node1", "type", "subfolder", "name", "Node 1", "created", "2023-01-02"]).await?;
    client.send_command_str("HSET", &["tree:data:leaf1", "type", "file", "name", "Leaf 1", "size", "1024", "created", "2023-01-03"]).await?;
    ...
    for counter in &counters {
        ...
        client.send_command_str("SET", &[counter, &value.to_string()]).await?;
    }
    
    // Increment each counter
    for counter in &counters {
        let response = client.send_command_str("INCR", &[counter]).await?;
        assert!(matches!(response, RespValue::Integer(_)), "INCR should return integer");
    }
    ...
    let response = client.send_command_str("EXISTS", &["counter:user:1"]).await?;
    assert_eq!(response, RespValue::Integer(1), "counter:user:1 should exist");
    
    let response = client.send_command_str("EXISTS", &["counter:user:2"]).await?;
    assert_eq!(response, RespValue::Integer(1), "counter:user:2 should exist");
    ...
    for day in 1..=7 {
        ...
        client.send_command_str(
            "ZADD", 
            &["timeseries:temperature", &timestamp.to_string(), &temperature.to_string()]
        ).await?;
    }
    ...
    */
    ...
    for key in &product_keys {
        client.send_command_str("SET", &[key, "product_data"]).await?;
    }
