...
mod type_mutation_tests {
    ...
    async fn test_string_type_consistency() {
        ...
        assert_eq!(
            key_type2, "string",
            "Key should still be string type after GET"
        );
        ...
        for i in 0..5 {
            ...
            assert_eq!(
                key_type, "string",
                "Key should remain string type after GET {}",
                i
            );
        }
    ...
    async fn test_base64_data_type_detection() {
        ...
        let test_cases = vec![
            ...
            (
                "JSON base64",
                general_purpose::STANDARD.encode(r#"{"test": "value"}"#.as_bytes()),
            ),
            (
                "Array-like base64",
                general_purpose::STANDARD.encode(r#"["item1", "item2"]"#.as_bytes()),
            ),
            (
                "Binary base64",
        ...
        for (desc, base64_data) in test_cases {
            ...
            for i in 0..3 {
                ...
                assert_eq!(
                    key_type, "string",
                    "{} should be string type (check {}), got: {}",
                    desc, i, key_type
                );
    ...
    async fn test_problematic_binary_patterns() {
        ...
        for (i, pattern) in problematic_patterns.iter().enumerate() {
            ...
            let set_result = engine
                .set(key.as_bytes().to_vec(), pattern.clone(), None)
                .await;
            assert!(
                set_result.is_ok(),
                "SET failed for binary pattern {}: {:?}",
                i,
                set_result
            );
            ...
            for check in 0..3 {
                ...
                assert_eq!(
                    key_type, "string",
                    "Binary pattern {} should be string type (check {}), got: {}",
                    i, check, key_type
                );
