...
mod debug_key_tests {
    ...
    async fn test_problematic_backend_key() {
        ...
        let mut large_json = String::from(r#"{"id": 1, "title": "Test Story", "content": "This is a test story with a lot of content to make it around 5012 characters long. "#);
        ...
        assert!(result.is_ok(), "Failed to set with problematic key: {:?}", result);
        ...
        assert!(result.is_ok(), "Failed to get with original key: {:?}", result);
        ...
        assert!(result_prefixed.is_ok(), "GET should succeed even for non-existent keys");
        
        match result_prefixed.unwrap() {
            RespValue::BulkString(None) => println!("✓ Prefixed key correctly returns nil"),
            other => panic!("Expected BulkString(None) for prefixed key, got: {:?}", other),
        }
    ...
    async fn test_very_long_key() {
        ...
        let set_args = vec![
            long_key.as_bytes().to_vec(),
            b"test_value".to_vec(),
        ];
        
        let result = string::set(&engine, &set_args).await;
        assert!(result.is_ok(), "Failed to set with 1024-byte key: {:?}", result);
        ...
        let set_args_long = vec![
            very_long_key.as_bytes().to_vec(),
            b"test_value".to_vec(),
        ];
    ...
    async fn test_chunked_json_simulation() {
        ...
        // Store chunks
        let result1 = string::set(&engine, &[chunk1_key.as_bytes().to_vec(), json_part1.as_bytes().to_vec()]).await;
        let result2 = string::set(&engine, &[chunk2_key.as_bytes().to_vec(), json_part2.as_bytes().to_vec()]).await;
