...
async fn test_redis_datatype_compatibility() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    client = new_client;
    let response = result?;
    assert_eq!(response, RespValue::BulkString(Some(b"string_value_suffix".to_vec())));
    ...
    // Create a set
    let sadd_args: Vec<&[u8]> = vec![
        b"SADD", b"set_key", b"member1", b"member2", b"member3"
    ];
    ...
    let del_args: Vec<&[u8]> = vec![
        b"DEL", 
        b"string_key", b"hash_key", b"list_key", 
        b"set_key", b"zset_key"
    ];
