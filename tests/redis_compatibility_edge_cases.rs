...
async fn test_redis_edge_cases() {
    ...
    let result = client
        .send_command_str("SET", &["key", "value", "EX", "invalid"])
        .await;
    ...
    match result {
        Ok(RespValue::Error(e)) => {
            ...
            assert!(
                e.contains("value is not an integer")
                    || e.contains("not a valid integer")
                    || e.contains("ERR")
            );
        }
    ...
    match result {
        ...
        Ok(unexpected) => panic!(
            "Expected error for GET on non-string, got: {:?}",
            unexpected
        ),
        Err(e) => {
    ...
    let result = client
        .send_command_str("SET", &["conflict", "value", "NX", "XX"])
        .await;
    ...
    match result {
        ...
        Ok(unexpected) => panic!(
            "Expected error for conflicting options, got: {:?}",
            unexpected
        ),
        Err(e) => {
    ...
    match result {
        ...
        Ok(unexpected) => panic!(
            "Expected error for too few arguments, got: {:?}",
            unexpected
        ),
        Err(e) => {
    ...
    match result {
        ...
        Ok(other) => {
            panic!(
                "Expected BulkString(None) or Error for non-existent key, got: {:?}",
                other
            );
        }
        Err(e) => {
    ...
    let result = client
        .send_command_str("HGET", &["nonexistent_hash", "field"])
        .await;
    match result {
        Ok(RespValue::BulkString(None)) => {
        ...
        Ok(other) => {
            panic!(
                "Expected BulkString(None) or Error for non-existent hash, got: {:?}",
                other
            );
        }
        Err(e) => {
    ...
    let result = client
        .send_command_str("LINDEX", &["nonexistent_list", "0"])
        .await;
    match result {
        Ok(RespValue::BulkString(None)) => {
        ...
        Ok(other) => {
            panic!(
                "Expected BulkString(None) or Error for non-existent list, got: {:?}",
                other
            );
        }
        Err(e) => {
    ...
    match result {
        ...
        Ok(other) => {
            panic!(
                "Expected Integer(-2) for TTL on non-existent key, got: {:?}",
                other
            );
        }
        Err(e) => {
...
async fn test_persist_command() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Set a key with TTL
    let response = client
        .send_command_str("SET", &["test_key", "value", "EX", "60"])
        .await?;
    ...
    let response = client
        .send_command_str("PERSIST", &["nonexistent_key"])
        .await?;
...
async fn test_data_type_edge_cases() {
    ...
    let result = client
        .send_command_str("HSET", &["hash_key", "field", "value"])
        .await;
    assert!(matches!(result, Ok(RespValue::Integer(1))));

    let result = client
        .send_command_str("LPUSH", &["list_key", "value"])
        .await;
    assert!(matches!(result, Ok(RespValue::Integer(1))));
    ...
    let result = client
        .send_command_str("ZADD", &["zset_key", "1", "value"])
        .await;
    assert!(matches!(result, Ok(RespValue::Integer(1))));
    ...
    match result {
        ...
        Ok(other) => {
            panic!(
                "Expected WRONGTYPE error or nil response but got: {:?}",
                other
            );
        }
        Err(e) => {
    ...
    match result {
        ...
        Ok(RespValue::Array(Some(arr))) => {
            println!(
                "Got array with {} items, which is acceptable for some implementations",
                arr.len()
            );
        ...
        Ok(other) => {
            panic!(
                "Expected WRONGTYPE error or array response but got: {:?}",
                other
            );
        }
        Err(e) => {
    ...
    let result = client
        .send_command_str("ZRANGE", &["set_key", "0", "-1"])
        .await;
    ...
    match result {
        ...
        Ok(other) => {
            panic!(
                "Expected WRONGTYPE error or empty array but got: {:?}",
                other
            );
        }
        Err(e) => {
