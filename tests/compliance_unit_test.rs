...
async fn test_redis_compliance_summary() {
    ...
    let test_commands = vec![
        // String commands
        ("SET", Command { name: "set".to_string(), args: vec![b"test".to_vec(), b"value".to_vec()] }),
        ("GET", Command { name: "get".to_string(), args: vec![b"test".to_vec()] }),
        ("STRLEN", Command { name: "strlen".to_string(), args: vec![b"test".to_vec()] }),
        ("APPEND", Command { name: "append".to_string(), args: vec![b"test".to_vec(), b"more".to_vec()] }),
        ("INCR", Command { name: "incr".to_string(), args: vec![b"counter".to_vec()] }),
        ("DECR", Command { name: "decr".to_string(), args: vec![b"counter".to_vec()] }),
        ("MGET", Command { name: "mget".to_string(), args: vec![b"test".to_vec()] }),
        ("MSET", Command { name: "mset".to_string(), args: vec![b"test2".to_vec(), b"value2".to_vec()] }),
        ("GETRANGE", Command { name: "getrange".to_string(), args: vec![b"test".to_vec(), b"0".to_vec(), b"2".to_vec()] }),
        ("SETRANGE", Command { name: "setrange".to_string(), args: vec![b"test".to_vec(), b"0".to_vec(), b"best".to_vec()] }),
        ("GETSET", Command { name: "getset".to_string(), args: vec![b"test".to_vec(), b"newval".to_vec()] }),
        ("MSETNX", Command { name: "msetnx".to_string(), args: vec![b"new1".to_vec(), b"val1".to_vec()] }),
        ("INCRBYFLOAT", Command { name: "incrbyfloat".to_string(), args: vec![b"float".to_vec(), b"1.5".to_vec()] }),
        
        // Hash commands
        ("HSET", Command { name: "hset".to_string(), args: vec![b"hash".to_vec(), b"field".to_vec(), b"value".to_vec()] }),
        ("HGET", Command { name: "hget".to_string(), args: vec![b"hash".to_vec(), b"field".to_vec()] }),
        ("HKEYS", Command { name: "hkeys".to_string(), args: vec![b"hash".to_vec()] }),
        ("HVALS", Command { name: "hvals".to_string(), args: vec![b"hash".to_vec()] }),
        ("HLEN", Command { name: "hlen".to_string(), args: vec![b"hash".to_vec()] }),
        ("HEXISTS", Command { name: "hexists".to_string(), args: vec![b"hash".to_vec(), b"field".to_vec()] }),
        ("HDEL", Command { name: "hdel".to_string(), args: vec![b"hash".to_vec(), b"field".to_vec()] }),
        ("HMGET", Command { name: "hmget".to_string(), args: vec![b"hash".to_vec(), b"field".to_vec()] }),
        ("HINCRBY", Command { name: "hincrby".to_string(), args: vec![b"hash".to_vec(), b"counter".to_vec(), b"1".to_vec()] }),
        ("HINCRBYFLOAT", Command { name: "hincrbyfloat".to_string(), args: vec![b"hash".to_vec(), b"float".to_vec(), b"1.5".to_vec()] }),
        ("HSETNX", Command { name: "hsetnx".to_string(), args: vec![b"hash".to_vec(), b"newf".to_vec(), b"newv".to_vec()] }),
        ("HSTRLEN", Command { name: "hstrlen".to_string(), args: vec![b"hash".to_vec(), b"field".to_vec()] }),
        
        // JSON commands
        ("JSON.SET", Command { name: "json.set".to_string(), args: vec![b"json".to_vec(), b"$".to_vec(), b"{\"key\":\"value\"}".to_vec()] }),
        ("JSON.GET", Command { name: "json.get".to_string(), args: vec![b"json".to_vec()] }),
        ("JSON.TYPE", Command { name: "json.type".to_string(), args: vec![b"json".to_vec()] }),
        ("JSON.OBJKEYS", Command { name: "json.objkeys".to_string(), args: vec![b"json".to_vec()] }),
        ("JSON.OBJLEN", Command { name: "json.objlen".to_string(), args: vec![b"json".to_vec()] }),
        ("JSON.ARRLEN", Command { name: "json.arrlen".to_string(), args: vec![b"arr".to_vec()] }),
        ("JSON.ARRAPPEND", Command { name: "json.arrappend".to_string(), args: vec![b"arr".to_vec(), b"$".to_vec(), b"1".to_vec()] }),
        ("JSON.ARRTRIM", Command { name: "json.arrtrim".to_string(), args: vec![b"arr".to_vec(), b"$".to_vec(), b"0".to_vec(), b"1".to_vec()] }),
        ("JSON.NUMINCRBY", Command { name: "json.numincrby".to_string(), args: vec![b"num".to_vec(), b"$".to_vec(), b"5".to_vec()] }),
        ("JSON.DEL", Command { name: "json.del".to_string(), args: vec![b"json".to_vec()] }),
        
        // Server commands
        ("PING", Command { name: "ping".to_string(), args: vec![] }),
        ("EXISTS", Command { name: "exists".to_string(), args: vec![b"test".to_vec()] }),
        ("DEL", Command { name: "del".to_string(), args: vec![b"test".to_vec()] }),
        ("TTL", Command { name: "ttl".to_string(), args: vec![b"test".to_vec()] }),
        ("TYPE", Command { name: "type".to_string(), args: vec![b"test".to_vec()] }),
    ];
    
    // Setup some test data first
    let setup_commands = vec![
        Command { name: "set".to_string(), args: vec![b"test".to_vec(), b"value".to_vec()] },
        Command { name: "set".to_string(), args: vec![b"counter".to_vec(), b"1".to_vec()] },
        Command { name: "set".to_string(), args: vec![b"float".to_vec(), b"1.0".to_vec()] },
        Command { name: "hset".to_string(), args: vec![b"hash".to_vec(), b"field".to_vec(), b"value".to_vec()] },
        Command { name: "hset".to_string(), args: vec![b"hash".to_vec(), b"counter".to_vec(), b"5".to_vec()] },
        Command { name: "hset".to_string(), args: vec![b"hash".to_vec(), b"float".to_vec(), b"2.5".to_vec()] },
        Command { name: "json.set".to_string(), args: vec![b"arr".to_vec(), b"$".to_vec(), b"[1,2,3]".to_vec()] },
        Command { name: "json.set".to_string(), args: vec![b"num".to_vec(), b"$".to_vec(), b"10".to_vec()] },
    ];
    ...
    assert!(compliance_percentage >= 80.0, 
        "Redis compliance should be at least 80%, got {:.1}%", compliance_percentage);
}