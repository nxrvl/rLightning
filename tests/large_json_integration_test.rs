...
async fn test_large_json_through_resp() {
    ...
    let flush_resp = client.send_command_str("FLUSHALL", &[]).await.expect("Failed to flush database");
    ...
    assert!(large_json.len() > 1000, "JSON string should be at least 1KB");
...
async fn test_error_reproduction() {
    ...
    assert!(large_json.len() > 1000, "Generated JSON should be at least 1KB");
