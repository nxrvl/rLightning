...
use crate::test_utils::{create_client, setup_test_server};
...
async fn test_basic_operations() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    assert_eq!(
        response,
        RespValue::BulkString(Some(b"expiry_value".to_vec()))
    );
...
async fn test_del_exists() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let exists_multi_args: Vec<&[u8]> = vec![b"EXISTS", b"del_key_1", b"del_key_2", b"nonexistent"];
    ...
    let del_multi_args: Vec<&[u8]> = vec![b"DEL", b"del_key_2", b"del_key_3", b"nonexistent"];
