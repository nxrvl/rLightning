...
async fn test_redis_protocol_compliance() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    assert!(
        ...
        "Expected response containing 'value'"
    );
    ...
    assert!(
        ...
        "Expected null bulk string response"
    );
    ...
    assert!(
        ...
        "Expected +OK response from JSON.SET"
    );
    ...
    assert!(
        ...
        "Expected array response from JSON.RESP"
    );
