...
mod large_json_tests {
    ...
    async fn test_large_json_value() {
        ...
        assert!(large_json.len() > 15000, "Generated JSON should be at least 15KB");
        ...
        assert!(result.is_ok(), "Failed to get large JSON value from engine: {:?}", result);
    ...
    async fn test_json_with_control_characters() {
        ...
        assert!(result.is_ok(), "Failed to get JSON with control characters from engine: {:?}", result);
