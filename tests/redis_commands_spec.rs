...
async fn test_redis_command_specifications() -> Result<(), Box<dyn std::error::Error + Send + Sync>>
{
    ...
    assert_eq!(
        response,
        RespValue::Integer(2),
        "DEL should return count of keys actually deleted"
    );
    ...
    assert_eq!(
        response,
        RespValue::Integer(1),
        "EXISTS should return count of keys that exist"
    );
    ...
    assert_eq!(
        response,
        RespValue::Integer(1),
        "EXPIRE should return 1 when key exists"
    );
    ...
    assert_eq!(
        response,
        RespValue::Integer(0),
        "EXPIRE should return 0 when key doesn't exist"
    );
    ...
    assert_eq!(
        response,
        RespValue::Integer(-1),
        "TTL should return -1 for persistent key"
    );
    ...
    if let RespValue::Integer(ttl) = response {
        assert!(
            ttl > 0 && ttl <= 10,
            "TTL should return >0 for key with expiry"
        );
    } else {
    ...
    assert_eq!(
        ...
        "SET NX should return nil if key exists"
    );
    ...
    assert_eq!(
        response,
        RespValue::BulkString(None),
        "SET XX should return nil if key doesn't exist"
    );
    ...
    assert_eq!(
        response,
        RespValue::Integer(1),
        "INCR should create key with value 1"
    );
    ...
    assert_eq!(
        response,
        RespValue::Integer(1),
        "HSET should return count of new fields only"
    );
    ...
    assert_eq!(
        response,
        RespValue::Integer(2),
        "LPUSH should return list length"
    );
    ...
    assert_eq!(
        response,
        RespValue::Integer(0),
        "LLEN on non-existent list should return 0"
    );
    ...
    assert_eq!(
        response,
        RespValue::Integer(2),
        "SADD should return count of new elements"
    );
    ...
    assert_eq!(
        response,
        RespValue::Integer(1),
        "SISMEMBER should return 1 for member"
    );
    ...
    assert_eq!(
        response,
        RespValue::Integer(0),
        "SISMEMBER should return 0 for non-member"
    );

    // ======== SORTED SET COMMANDS ========

    // ZADD - Should return number of elements added (not updated)
    let zadd_args: Vec<&[u8]> = vec![b"ZADD", b"zset_key", b"10", b"member1", b"20", b"member2"];
    let response = client.send_command(zadd_args).await?;
    assert_eq!(
        response,
        RespValue::Integer(2),
        "ZADD should return count of new elements"
    );
    ...
    assert_eq!(
        response,
        RespValue::Integer(1),
        "ZADD should return count of new elements only"
    );
    ...
    assert_eq!(
        response,
        RespValue::BulkString(Some(b"15".to_vec())),
        "ZSCORE should return score as string"
    );
