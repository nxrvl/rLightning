...
mod data_corruption_tests {
    ...
    async fn test_base64_json_corruption_scenario() {
        ...
        assert!(exists_result.is_ok(), "EXISTS check failed: {:?}", exists_result);
        ...
        let type_result = engine.get_type(key.as_bytes()).await;
        assert!(type_result.is_ok(), "TYPE check after first GET failed: {:?}", type_result);
        let key_type = type_result.unwrap();
        println!("Key type after first GET: {}", key_type);
        assert_eq!(key_type, "string", "Key should still be string type after first GET");
        ...
        match get_result2.unwrap() {
            RespValue::BulkString(Some(data)) => {
                ...
                assert_eq!(retrieved, prefixed_data, "Second GET should return same data");
        ...
        let type_result = engine.get_type(key.as_bytes()).await;
        assert!(type_result.is_ok(), "TYPE check after second GET failed: {:?}", type_result);
        let key_type = type_result.unwrap();
        println!("Key type after second GET: {}", key_type);
        assert_eq!(key_type, "string", "Key should still be string type after second GET");
        
        // Step 4: Multiple rapid GETs to test for race conditions
        println!("\n--- Step 4: Rapid GET operations ---");
        for i in 0..10 {
            let get_result = string::get(&engine, &get_args).await;
            assert!(get_result.is_ok(), "Rapid GET {} failed: {:?}", i, get_result);
    ...
    async fn test_concurrent_set_get_operations() {
        ...
        for i in 0..5 {
            ...
            let handle = tokio::spawn(async move {
                ...
                assert!(set_result.is_ok(), "Concurrent SET {} failed: {:?}", i, set_result);
                
                // Immediate GET
                let get_args = vec![key_clone.as_bytes().to_vec()];
                let get_result = string::get(&engine_clone, &get_args).await;
                assert!(get_result.is_ok(), "Concurrent GET {} failed: {:?}", i, get_result);
                
                match get_result.unwrap() {
                    RespValue::BulkString(Some(data)) => {
                        let retrieved = String::from_utf8_lossy(&data);
                        assert_eq!(retrieved, data_clone, "Concurrent operation {} data mismatch", i);
                    },
                    other => panic!("Concurrent operation {} returned unexpected: {:?}", i, other),
                }
    ...
    async fn test_large_base64_data_corruption() {
        ...
        for i in 0..1000 {
            large_json.push_str(&format!(r#"{{"id": {}, "data": "item_{}_data_with_more_content"}},"#, i, i));
        }
        ...
        let set_result = string::set(&engine, &set_args).await;
        assert!(set_result.is_ok(), "Large data SET failed: {:?}", set_result);
        ...
        for i in 0..5 {
            let get_args = vec![key.as_bytes().to_vec()];
            let get_result = string::get(&engine, &get_args).await;
            assert!(get_result.is_ok(), "Large data GET {} failed: {:?}", i, get_result);
            
            match get_result.unwrap() {
                RespValue::BulkString(Some(data)) => {
                    let retrieved = String::from_utf8_lossy(&data);
                    assert_eq!(retrieved.len(), prefixed_data.len(), "Large data size mismatch on GET {}", i);
                    assert_eq!(retrieved, prefixed_data, "Large data content mismatch on GET {}", i);
                },
