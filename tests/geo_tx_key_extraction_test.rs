...
#[tokio::test]
async fn test_watch_multi_geosearchstore_integrity()
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let _ = client1
        .send_command_str("GEOADD", &["places", "13.361389", "38.115556", "Palermo"])
        .await?;
    let _ = client1
        .send_command_str("GEOADD", &["places", "15.087269", "37.502669", "Catania"])
        .await?;
    let _ = client1
        .send_command_str("GEOADD", &["places", "2.349014", "48.864716", "Paris"])
        .await?;
    ...
    match &exec_result {
        RespValue::Array(Some(results)) => {
            ...
            if let RespValue::Integer(count) = &results[0] {
                assert!(
                    *count >= 1,
                    "Should have stored at least 1 result (Palermo/Catania are near)"
                );
                println!("  GEOSEARCHSTORE stored {} elements", count);
            } else {
                panic!(
                    "Expected integer result from GEOSEARCHSTORE, got: {:?}",
                    results[0]
                );
            }
        }
    ...
    if let RespValue::Integer(n) = card {
        assert!(
            n >= 1,
            "nearby should have at least 1 member after GEOSEARCHSTORE"
        );
    ...
    let _ = client2
        .send_command_str("SET", &["nearby", "interference"])
        .await?;
    ...
    match &exec_result {
        RespValue::Array(Some(results)) => {
            ...
            if let RespValue::Integer(count) = &results[0] {
                assert!(
                    *count >= 1,
                    "GEORADIUS STORE should store at least 1 result"
                );
    ...
    let card = client1
        .send_command_str("ZCARD", &["radius_result"])
        .await?;
    if let RespValue::Integer(n) = card {
