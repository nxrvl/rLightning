...
async fn test_set_get_problematic_data() {
    ...
    // Connect to server
    let mut stream = TcpStream::connect("127.0.0.1:16398")
        .await
    ...
    assert!(
        set_response.contains("+OK"),
        "SET should succeed with OK response"
    );
    ...
    assert!(
        get_response.contains(&expected_response),
...
async fn test_multiple_problematic_patterns() {
    ...
    for (i, pattern) in test_patterns.iter().enumerate() {
        ...
        let result = storage
            .set(key.as_bytes().to_vec(), pattern.as_bytes().to_vec(), None)
            .await;
        assert!(
            result.is_ok(),
        ...
        assert!(
            data.is_some(),
            "Retrieved data should exist for pattern: {}",
            pattern
        );
        assert_eq!(
            data.unwrap(),
...
async fn test_concurrent_set_get_operations() {
    ...
    for i in 0..20 {
        ...
        let handle = tokio::spawn(async move {
            ...
            assert!(
                set_result.is_ok(),
                "Concurrent SET should succeed for key: {}",
                key
            );
            ...
            assert!(
                get_result.is_ok(),
                "Concurrent GET should succeed for key: {}",
                key
            );
            ...
            assert!(
                retrieved.is_some(),
                "Retrieved data should exist for key: {}",
                key
            );
            assert_eq!(
                retrieved.unwrap(),
                problematic_data.as_bytes(),
                "Data should match exactly for key: {}",
                key
            );
...
fn test_resp_value_serialization_with_problematic_data() {
    ...
    for data in test_cases {
        ...
        let expected = format!("${}\r\n{}\r\n", data.len(), data);
        assert_eq!(
            serialized_str, expected,
            "Serialization should be correct for: {}",
            data
        );
...
async fn test_large_json_with_resp_patterns() {
    ...
    let result = storage
        .set(key.clone(), large_json.as_bytes().to_vec(), None)
        .await;
    ...
    assert_eq!(
        data.unwrap(),
        large_json.as_bytes(),
        "Large JSON should match exactly"
    );
}
