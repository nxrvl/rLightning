...
async fn test_debug_logging() {
    ...
    let result = client.send_command_str("PING", &["Hello"]).await.expect("PING command should succeed");
    assert!(matches!(result, RespValue::BulkString(Some(_))));
    
    // Send a SET command
    let result = client.send_command_str("SET", &["mykey", "myvalue"]).await.expect("SET command should succeed");
    assert_eq!(result, RespValue::SimpleString("OK".to_string()));
    
    // Send a GET command
    let result = client.send_command_str("GET", &["mykey"]).await.expect("GET command should succeed");
    match result {
