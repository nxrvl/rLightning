...
fn test_process_json_for_serialization() {
    ...
    if let Value::Object(map) = &processed {
        assert_eq!(map.get("created_at").unwrap(), &json!("2023-04-01T12:34:56"));
        assert_eq!(map.get("updated_at").unwrap(), &json!("2023-04-02T10:20:30"));
        
        if let Value::Object(metadata) = map.get("metadata").unwrap() {
            assert_eq!(metadata.get("timestamp").unwrap(), &json!("2023-04-03T15:45:12"));
        } else {
...
fn test_convert_from_iso8601() {
    ...
    let space_fmt = rlightning::utils::datetime::convert_from_iso8601(
        iso8601, 
    ...
    // To Python "at" format
    let python_fmt = rlightning::utils::datetime::convert_from_iso8601(
        iso8601, 
    ...
    let back_to_iso = rlightning::utils::datetime::convert_from_iso8601(
        iso8601, 
