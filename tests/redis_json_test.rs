...
async fn test_redis_json_commands() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let json_obj =
        r#"{"name":"John","age":30,"address":{"city":"New York"},"skills":["Rust","Redis"]}"#;
    let result = client
        .send_command_str("JSON.SET", &["user:1", ".", json_obj])
        .await?;
    ...
    if let RespValue::BulkString(Some(bytes)) = result {
        ...
        panic!(
            "Expected BulkString response from JSON.GET, got: {:?}",
            result
        );
    }
    ...
    let result = client
        .send_command_str("JSON.GET", &["user:1", "name"])
        .await?;
    if let RespValue::BulkString(Some(bytes)) = result {
        let json_str = String::from_utf8(bytes)?;
        ...
        panic!(
            "Expected BulkString response from JSON.GET with path, got: {:?}",
            result
        );
    }
    ...
    let result = client
        .send_command_str("JSON.TYPE", &["user:1", "name"])
        .await?;
    assert_eq!(result, RespValue::BulkString(Some(b"string".to_vec())));

    let result = client
        .send_command_str("JSON.TYPE", &["user:1", "skills"])
        .await?;
    assert_eq!(result, RespValue::BulkString(Some(b"array".to_vec())));

    // Test JSON.SET for updating a specific path
    println!("Testing JSON.SET for updating a specific path");
    let result = client
        .send_command_str("JSON.SET", &["user:1", "age", "31"])
        .await?;
    assert_eq!(result, RespValue::SimpleString("OK".to_string()));

    // Verify the update worked
    let result = client
        .send_command_str("JSON.GET", &["user:1", "age"])
        .await?;
    if let RespValue::BulkString(Some(bytes)) = result {
        let json_str = String::from_utf8(bytes)?;
        ...
        panic!(
            "Expected BulkString response from JSON.GET after update, got: {:?}",
            result
        );
    }
    ...
    let result = client
        .send_command_str(
            "JSON.ARRAPPEND",
            &["user:1", "skills", "\"Python\"", "\"JavaScript\""],
        )
        .await?;
    if let RespValue::Integer(len) = result {
        ...
        panic!(
            "Expected Integer response from JSON.ARRAPPEND, got: {:?}",
            result
        );
    }
    ...
    let result = client
        .send_command_str("JSON.GET", &["user:1", "skills"])
        .await?;
    if let RespValue::BulkString(Some(bytes)) = result {
        let json_str = String::from_utf8(bytes)?;
        ...
        panic!(
            "Expected BulkString response from JSON.GET for skills, got: {:?}",
            result
        );
    }
    ...
    let result = client
        .send_command_str("JSON.ARRTRIM", &["user:1", "skills", "1", "2"])
        .await?;
    if let RespValue::Integer(len) = result {
        assert_eq!(len, 2); // ["Redis", "Python"] - length 2
    } else {
        panic!(
            "Expected Integer response from JSON.ARRTRIM, got: {:?}",
            result
        );
    }
