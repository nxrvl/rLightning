...
async fn test_redis_advanced_compatibility() -> Result<(), Box<dyn std::error::Error + Send + Sync>>
{
    ...
    if let RespValue::Array(Some(values)) = response {
        ...
        assert!(
            matches!(values[3], RespValue::BulkString(None)),
            "Fourth value should be nil"
        );
    } else {
    ...
    let set_args = &[
        "bits",
        "\u{0001}\u{0000}\u{0000}\u{0000}\u{0000}\u{0000}\u{0000}",
    ];
    ...
    assert_eq!(
        response,
        RespValue::Integer(1),
        "EXPIRE should return 1 for success"
    );
    ...
    if let RespValue::Integer(ttl) = response {
        assert!(
            ttl > 0 && ttl <= 1,
            "TTL should return a value between 0 and 1 seconds, got {}",
            ttl
        );
    } else {
    ...
    assert_eq!(
        ...
        "Key should still have its value"
    );
    ...
    assert_eq!(
        ...
        "Key should be expired and return nil"
    );
    ...
    // Initialize list
    client
        .send_command_str("LPUSH", &["list", "C", "B", "A"])
        .await?;
    ...
    assert_eq!(
        response,
        RespValue::Integer(4),
        "LPUSH should return the new list length"
    );
    ...
    let response = client
        .send_command_str("LRANGE", &["list", "0", "-1"])
        .await?;
    ...
    client
        .send_command_str(
            "HSET",
            &["user:hash", "name", "John", "age", "30", "city", "New York"],
        )
        .await?;
    ...
    // Test HMGET
    let response = client
        .send_command_str("HMGET", &["user:hash", "name", "age", "nonexistent"])
        .await?;
    if let RespValue::Array(Some(values)) = response {
    ...
    // Test HEXISTS
    let response = client
        .send_command_str("HEXISTS", &["user:hash", "name"])
        .await?;
    assert_eq!(
        response,
        RespValue::Integer(1),
    ...
    let response = client
        .send_command_str("HEXISTS", &["user:hash", "nonexistent"])
        .await?;
    assert_eq!(
        response,
        RespValue::Integer(0),
        "HEXISTS should return 0 for non-existing field"
    );
    ...
    // Test ZCOUNT
    let response = client
        .send_command_str("ZCOUNT", &["scores", "100", "160"])
        .await?;
    assert_eq!(response, RespValue::Integer(2), "ZCOUNT should return 2");

    // Test ZINCRBY
    let response = client
        .send_command_str("ZINCRBY", &["scores", "50", "Alice"])
        .await?;
    assert_eq!(
        response,
        RespValue::BulkString(Some(b"150".to_vec())),
        "ZINCRBY should return new score 150"
    );
    ...
    let response = client
        .send_command_str("ZRANGE", &["scores", "0", "-1", "WITHSCORES"])
        .await?;
    if let RespValue::Array(Some(members_scores)) = response {
        assert_eq!(
            members_scores.len(),
            4,
            "ZRANGE with WITHSCORES should return 4 values"
        );
        ...
        if let RespValue::BulkString(Some(m1)) = &members_scores[0] {
            ...
            assert!(
                m1_str == "Alice" || m1_str == "Charlie",
                "First member should be Alice or Charlie"
            );
        }
        ...
        if let RespValue::BulkString(Some(m2)) = &members_scores[2] {
            ...
            assert!(
                m2_str == "Alice" || m2_str == "Charlie",
                "Second member should be Alice or Charlie"
            );
        }
    ...
    // Initialize sets
    client
        .send_command_str("SADD", &["set1", "a", "b", "c", "d"])
        .await?;
    client
        .send_command_str("SADD", &["set2", "c", "d", "e", "f"])
        .await?;
    ...
    // Test SINTERSTORE
    let response = client
        .send_command_str("SINTERSTORE", &["result", "set1", "set2"])
        .await?;
    assert_eq!(
        response,
        RespValue::Integer(2),
        "SINTERSTORE should return 2"
    );
    ...
    assert!(
        ...
        "SET with missing value should return error"
    );

    // Test operation on wrong type
    client
        .send_command_str("SET", &["string_key", "value"])
        .await?;
    let response = client
        .send_command_str("HGET", &["string_key", "field"])
        .await?;
    assert!(
