...
async fn test_redis_geo_commands() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    assert_eq!(
        result,
        RespValue::Integer(4),
        "GEOADD should return 4 when 4 locations are added"
    );
    ...
    let result = client
        .send_command_str("GEODIST", &["locations", "Palermo", "Catania", "km"])
        .await?;
    match result {
        RespValue::BulkString(Some(bytes)) => {
            ...
            let distance_val: f64 = distance.parse()?;
            assert!(
                distance_val > 160.0 && distance_val < 170.0,
                "Distance between Palermo and Catania should be around 166.3 km"
            );
        }
        RespValue::Error(_) => {
        ...
        _ => {
            panic!(
                "Unexpected response type from GEODIST with unit: {:?}",
                result
            );
        }
    }
    ...
    let result = client
        .send_command_str("GEODIST", &["locations", "Palermo", "Nonexistent"])
        .await?;
    assert_eq!(
        result,
        RespValue::BulkString(None),
        "GEODIST should return nil for non-existing member"
    );
    ...
    let result = client
        .send_command_str("GEOHASH", &["locations", "Rome"])
        .await?;
    match result {
        RespValue::Array(Some(hashes)) => {
            assert_eq!(
                hashes.len(),
                1,
                "GEOHASH should return 1 hash for 1 requested member"
            );
            if let RespValue::BulkString(Some(hash_bytes)) = &hashes[0] {
                ...
                assert!(
                    hash.starts_with("sr"),
                    "Geohash for Rome should start with sr"
                );
            } else {
    ...
    let result = client
        .send_command_str("GEOHASH", &["locations", "Rome", "Paris", "Nonexistent"])
        .await?;
    match result {
        RespValue::Array(Some(hashes)) => {
            assert_eq!(
                hashes.len(),
                3,
                "GEOHASH should return 3 results for 3 requested members"
            );
            // Third result should be nil for non-existing member
            assert_eq!(
                hashes[2],
                RespValue::BulkString(None),
                "GEOHASH should return nil for non-existing member"
            );
        }
        RespValue::Error(_) => {
        ...
        _ => {
            panic!(
                "Unexpected response type from GEOHASH with multiple members: {:?}",
                result
            );
        }
    }
    ...
    let result = client
        .send_command_str("GEOPOS", &["locations", "Rome"])
        .await?;
    match result {
        RespValue::Array(Some(positions)) => {
            assert_eq!(
                positions.len(),
                1,
                "GEOPOS should return 1 position for 1 requested member"
            );
            ...
            if let RespValue::Array(Some(coords)) = &positions[0] {
                assert_eq!(
                    coords.len(),
                    2,
                    "Each position should contain longitude and latitude"
                );
                ...
                if let RespValue::BulkString(Some(lon_bytes)) = &coords[0] {
                    ...
                    assert!(
                        (lon - 12.496366).abs() < 0.1,
                        "Rome longitude should be close to 12.496366"
                    );
                } else {
                ...
                if let RespValue::BulkString(Some(lat_bytes)) = &coords[1] {
                    ...
                    assert!(
                        (lat - 41.902782).abs() < 0.1,
                        "Rome latitude should be close to 41.902782"
                    );
                } else {
    ...
    let result = client
        .send_command_str(
            "GEORADIUS",
            &["locations", "13.583333", "37.316667", "200", "km"],
        )
        .await?;
    match result {
        RespValue::Array(Some(members)) => {
            assert!(
                members.len() >= 2,
                "GEORADIUS should find at least Palermo and Catania within 200 km"
            );
            ...
            assert!(
                found_palermo,
                "GEORADIUS should find Palermo within 200 km of specified point"
            );
            assert!(
                found_catania,
                "GEORADIUS should find Catania within 200 km of specified point"
            );
        }
        RespValue::Error(ref err) => {
            if err.contains("unknown command") || err.contains("not implemented") {
                println!(
                    "GEORADIUS command is not implemented, skipping remaining georadius tests"
                );
            } else {
    ...
    let result = client
        .send_command_str("ZRANGE", &["locations", "0", "-1"])
        .await?;
    ...
    let result = client
        .send_command_str("EXPIRE", &["locations", "1"])
        .await?;
    assert_eq!(
        result,
        RespValue::Integer(1),
        "EXPIRE should return 1 when successful"
    );
    ...
    client
        .send_command_str(
            "GEOADD",
            &["locations2", "13.361389", "38.115556", "Palermo"],
        )
        .await?;
    ...
    assert_eq!(
        result,
        RespValue::Integer(1),
        "DEL should return count of keys deleted"
    );
