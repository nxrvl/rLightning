...
async fn test_concurrent_access() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    assert!(success_count as f64 / total_ops as f64 > 0.95, 
    ...
    assert!(!all_keys.is_empty(), "Expected to find keys created during stress test");
...
async fn test_simple_throughput() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn try_connect(addr: SocketAddr) -> Result<Client, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn try_command(addr: SocketAddr, mut client: Client, cmd: Vec<&[u8]>) -> (Client, Result<RespValue, Box<dyn std::error::Error + Send + Sync>>) {
    ...
    while i < NUM_OPS && successful_ops < NUM_OPS {
        ...
        match set_result {
            Ok(result) => {
                ...
                match get_result {
                    Ok(result) => {
                        assert_eq!(result, RespValue::BulkString(Some(value.as_bytes().to_vec())));
                        successful_ops += 1;
                        i += 1;
                    }
                    Err(e) => {
