...
use rlightning::networking::resp::{RespValue, RespError};
...
async fn test_resp_protocol_data_command_separation() {
    ...
    assert!(result.is_err(), "Parser should reject data that looks like stored content");
    
    if let Err(e) = result {
        let error_msg = e.to_string();
        assert!(
            error_msg.contains("Data corruption detected") || error_msg.contains("Invalid RESP command"),
            "Error should indicate data corruption detection, got: {}", error_msg
        );
    }
}
...
async fn test_stateful_parser_prevents_confusion() {
    ...
    let result = parser.parse_with_state(&mut buffer);
    assert!(result.is_err(), "Stateful parser should reject data in command mode");
}
...
async fn test_large_json_storage_without_corruption() {
    ...
    }}"#, "x".repeat(10000), 15000, "y".repeat(5000));
    ...
    assert!(result.is_ok(), "Should be able to store large JSON data: {:?}", result);
    
    // Retrieve the data
    let retrieved = storage.get(&key).await;
    assert!(retrieved.is_ok(), "Should be able to retrieve large JSON data: {:?}", retrieved);
    
    let retrieved_value = retrieved.unwrap();
    assert!(retrieved_value.is_some(), "Retrieved value should exist");
    assert_eq!(retrieved_value.unwrap(), value, "Retrieved data should match stored data exactly");
}
...
async fn test_bulk_string_with_exact_length() {
    ...
    if let Ok(Some(RespValue::BulkString(Some(data)))) = result {
        assert_eq!(data, test_data.as_bytes(), "Bulk string data should match exactly");
    } else {
    ...
    for pattern in problematic_patterns {
        ...
        match result {
            ...
            Err(e) => {
                ...
                assert!(!e.to_string().is_empty(), "Error message should be descriptive");
            }
        }
    }
}
...
async fn test_set_get_with_special_data() {
    ...
    for (key, value) in test_cases {
        ...
        assert!(set_result.is_ok(), "Should be able to store data that looks like RESP commands");
        
        // Retrieve the data
        let get_result = storage.get(&key).await;
        assert!(get_result.is_ok(), "Should be able to retrieve special data");
        
        let retrieved = get_result.unwrap();
        assert!(retrieved.is_some(), "Retrieved data should exist");
        assert_eq!(retrieved.unwrap(), value, "Retrieved data should match stored data exactly");
    }
}
...
async fn test_concurrent_operations_with_large_data() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            // Set data
            storage_clone.set(key.clone(), large_data.clone(), None).await.unwrap();
...
fn test_resp_value_serialization_safety() {
    ...
    for value in test_cases {
        ...
        assert!(serialized.is_ok(), "Serialization should succeed for: {:?}", value);
