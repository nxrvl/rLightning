...
use rlightning::command::types::string::{set, get};
...
async fn test_large_b64json_data() {
    ...
    if let Ok(RespValue::BulkString(Some(retrieved_data))) = get_result {
        let retrieved_str = String::from_utf8_lossy(&retrieved_data);
        assert_eq!(retrieved_str, large_b64json, "Large B64JSON data should match exactly");
        println!("✅ Large B64JSON test passed! Size: {} bytes", large_b64json.len());
    } else {
...
async fn test_multiple_set_get_operations() {
    ...
    for (key, expected_value) in &test_data {
        ...
        if let Ok(RespValue::BulkString(Some(data))) = result {
            ...
            assert_eq!(retrieved, *expected_value, "Data should match for key: {}", key);
        } else {
