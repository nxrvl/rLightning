...
async fn test_large_json_protocol_handling() -> Result<(), Box<dyn std::error::Error + Send + Sync>>
{
    ...
    // Print size info
    println!(
        "Large JSON size: {} bytes ({:.2} MB)",
        large_json.len(),
        large_json.len() as f64 / (1024.0 * 1024.0)
    );
    assert!(
        large_json.len() > 10 * 1024 * 1024,
        "JSON should be at least 10MB"
    );
    ...
    let result = client
        .send_command_str("JSON.SET", &["large_json", ".", &large_json])
        .await?;
    assert_eq!(
        result,
    ...
    if let RespValue::BulkString(Some(bytes)) = result {
        println!(
            "Successfully retrieved large JSON, size: {} bytes",
            bytes.len()
        );
        assert!(
            bytes.len() > 10 * 1024 * 1024,
            "Retrieved JSON should be at least 10MB"
        );
    ...
    println!(
        "Setting JSON with escaped characters, size: {} bytes",
        json_with_escapes.len()
    );

    // Store JSON with escaped characters
    let result = client
        .send_command_str("JSON.SET", &["escaped_json", ".", json_with_escapes])
        .await?;
    assert_eq!(
        result,
        RespValue::SimpleString("OK".to_string()),
    ...
    let result = client
        .send_command_str("JSON.GET", &["escaped_json"])
        .await?;
    if let RespValue::BulkString(Some(bytes)) = result {
        println!(
            "Successfully retrieved JSON with escaped characters, size: {} bytes",
            bytes.len()
        );
        ...
        let parsed: serde_json::Value =
            serde_json::from_str(&retrieved_str).expect("Retrieved JSON should be valid");
    ...
    let result = client
        .send_command_str("JSON.SET", &["invalid_json", ".", invalid_json])
        .await?;
    match result {
    ...
    let result = client
        .send_command_str("JSON.GET", &["large_json", &path])
        .await?;
    if let RespValue::BulkString(Some(bytes)) = result {
    ...
    let result = client
        .send_command_str("JSON.SET", &["large_json", update_path, "\"red\""])
        .await?;
    ...
    let result = client
        .send_command_str("JSON.SET", &["large_json2", ".", &large_json])
        .await?;
    assert_eq!(
        result,
        RespValue::SimpleString("OK".to_string()),
