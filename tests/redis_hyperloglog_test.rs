...
async fn test_redis_hyperloglog_commands() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let result = client
        .send_command_str("PFADD", &["hll1", "element1"])
        .await?;
    ...
    let result = client
        .send_command_str("PFADD", &["hll1", "element2", "element3", "element4"])
        .await?;
    assert!(
        matches!(result, RespValue::Integer(1)),
        "PFADD should return 1 when at least one element is added"
    );

    // Test PFADD with duplicate elements (should return 0 when no new elements are added)
    let result = client
        .send_command_str("PFADD", &["hll1", "element1", "element2"])
        .await?;
    assert_eq!(
        result,
        RespValue::Integer(0),
        "PFADD should return 0 when no new elements are added"
    );
    ...
    if let RespValue::Integer(count) = result {
        assert_eq!(
            count, 4,
            "PFCOUNT should return approximated cardinality of 4"
        );
    } else if let RespValue::Error(err) = &result {
        if err.contains("unknown command") || err.contains("not implemented") {
    ...
    client
        .send_command_str(
            "PFADD",
            &["hll2", "element3", "element4", "element5", "element6"],
        )
        .await?;

    // Test PFCOUNT with multiple keys
    let result = client
        .send_command_str("PFCOUNT", &["hll1", "hll2"])
        .await?;
    if let RespValue::Integer(count) = result {
        assert_eq!(
            count, 6,
            "PFCOUNT should return approximated cardinality of 6 for merged HyperLogLogs"
        );
    } else {
        println!(
            "Multi-key PFCOUNT may not be implemented, skipping: {:?}",
            result
        );
    }
    ...
    assert_eq!(
        result,
        RespValue::Integer(0),
        "PFCOUNT should return 0 for non-existing key"
    );
    ...
    // Test PFMERGE
    let result = client
        .send_command_str("PFMERGE", &["hll3", "hll1", "hll2"])
        .await?;
    ...
    if let RespValue::Integer(count) = result {
        assert_eq!(
            count, 6,
            "PFCOUNT should return approximate cardinality of 6 for merged HyperLogLog"
        );
    } else {
        panic!(
            "Unexpected response type from PFCOUNT after merge: {:?}",
            result
        );
    }
    ...
    assert!(
        ...
        "PFADD without key should return error"
    );
    ...
    assert!(
        matches!(result, RespValue::Error(_)),
        "PFCOUNT without key should return error"
    );

    // Test PFMERGE with only destination (valid in Redis - creates empty HLL or no-op)
    let result = client
        .send_command_str("PFMERGE", &["destination_only"])
        .await?;
    assert!(
