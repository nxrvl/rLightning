...
- [x] Redesign site theme with dark color scheme: deep navy background (#0a0e14), coral accent color (#E8553E), modern typography (IBM Plex Sans body, Fira Code for code)
- [x] Build hero section on index.md: large headline with gradient glow effects, tagline, CTA buttons (Get Started, GitHub)
- [x] Create "Why rLightning" section: 4-column responsive grid with key selling points (performance, Redis compatibility, Rust safety, modern architecture)
- [x] Create "Features" section: card-based layout showcasing capabilities (each data type, clustering, persistence, pub/sub, scripting)
- [x] Create "How It Works" section: step-by-step walkthrough with code examples showing Redis CLI usage
- [x] Create "Benchmark Results" section: visual comparison charts (rLightning vs Redis) with p50/p95/p99 latency and throughput numbers
- [x] Create "Installation" section: multi-method install (cargo, docker, binary download) with tabbed code blocks
- [x] Add glassmorphism navigation bar with backdrop blur, logo, and section links
- [x] Implement CSS animations: fade-in on scroll (intersection observer), hover transitions on cards, subtle animated dot-grid background pattern
- [x] Configure MkDocs Material theme overrides for dark palette, custom fonts, and extended navigation
- [x] Update command reference pages: add new command categories (bitmap, hyperloglog, geo, streams, transactions, scripting, ACL, cluster, sentinel) with usage examples and compatibility notes
- [x] Update architecture.md with cluster and sentinel architecture diagrams
- [x] Add benchmark results page with detailed comparison data and methodology
- [x] Ensure responsive design: grid layouts adapt from multi-column to single-column on mobile
- [x] Update CLAUDE.md with new commands, features, and architecture notes
- [x] Update README.md with new feature list
